...
firepad.Firepad = (function(global) {
  ...
  var FirebaseAdapter = firepad.WebSocketAdapter; // TODO: Rename this jazz
  ...
  function Firepad(ref, place, options) {
    ...
    } else if (ace && place instanceof ace.EditSession) {
      this.ace_ = this.editor_ = place;
      curValue = this.ace_.getValue();
      if (curValue !== '') {
        throw new Error("Can't initialize Firepad with an ACE session that already contains text.");
      }
    } else {
    ...
    // Collect OT changes to apply as a batch, they will be an atomic unit that
    // is either completely applied on the server side or not
    place.batchChanges = function(fn) {
      var previousApply = self.client_.applyClient;
      var pendingOperations = [];
      self.client_.applyClient = function(operation) {
        pendingOperations.push(operation);
      };
      fn();
      self.client_.applyClient = previousApply;

      // Compose operations into a single operation
      var operation = pendingOperations.reduce(function(prev, operation) {
        return prev.compose(operation);
      });
      self.client_.applyClient(operation);
    }
  ...
  Firepad.prototype.getText = function() {
    ...
    if (this.codeMirror_)
      ...
      return this.ace_.getValue();
  };

  Firepad.prototype.setText = function(textPieces) {
    if (this.ace_) {
      return this.ace_.setValue(textPieces);
    } else {
